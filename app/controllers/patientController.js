...
module.exports = {
    ...
    updatePatient: function(req, res, next) {
        ...
        .catch(() => {
            res.status(HTTP_CODE.NOT_FOUND).send(httpBuilder.constructHttpResponse(ROUTE, HTTP_METHOD.PUT, HTTP_CODE.NOT_FOUND));    
            next();
        });
    },
    ...
    deletePatient: function(req, res, next) {
        ...
        .then((result) => {
            res.status(HTTP_CODE.OK).send(httpBuilder.constructHttpResponse(ROUTE, HTTP_METHOD.DELETE, HTTP_CODE.OK, result));
            next();
        })
        .catch(() => {
            res.status(HTTP_CODE.NOT_FOUND).send(httpBuilder.constructHttpResponse(ROUTE, HTTP_METHOD.DELETE, HTTP_CODE.ERROR));
            next();
        });
    }
};